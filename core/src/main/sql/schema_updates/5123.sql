...
5123,
current_timestamp,
'reset status of all plates to "Not specified" (ICCB-L only)';

/* For ICCB-L only, which is using PlateBatchUpdater to set the status of all existing plates, and so we want all plates' status to be reset before this batch update is performed. */
...
select rpsa.activity_id, 0, now(), '''Plate Status Update'' activity created by database migration to reset plate''s status to ''Not specified''.', now(), -1, -1
