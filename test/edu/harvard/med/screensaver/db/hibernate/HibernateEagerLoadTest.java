...
public class HibernateEagerLoadTest extends AbstractSpringTest
{
   ...
   * left-join example, since we're not inspecting any of the values of the child
  ...
  public void testLoadVersusFetch() throws Exception
  {
    ...
    genericEntityDao.findEntitiesByHql(Library.class,
                                       "select distinct l from Library l left join       l.wells w left join       w.silencingReagents where l.libraryName = ?",
    ...
    genericEntityDao.findEntitiesByHql(Library.class,
                                       "select distinct l from Library l left join fetch l.wells w left join fetch w.silencingReagents where l.libraryName = ?",
                                       LIBRARY_NAME)).iterator().next();
  ...
  // this test should pass as long as the classpath contains our modified
