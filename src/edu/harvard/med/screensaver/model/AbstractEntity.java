...
/**
 * An abstract superclass for the entity beans in the object model. Provides an abstract
 * method {@link #getEntityId()} to define an entity id for all entity classes.
 *
 * <p>
 *
 * Provides
 * abstract method {@link #acceptVisitor(AbstractEntityVisitor)} to implement a visitor
 * pattern over the entity classes. The visitor pattern is currently used to implement
 * a core part of the data access policy for the entity model. In the future, we may also
 * use a similar AOP-style visitor pattern to implement business rule violation checks.
 *
 * <p>
 *
 * Provides various helper methods, including a {@link #isEquivalent(AbstractEntity) a way
 * to compare to entities for equivalence}, and methods for truncating dates. (These date
 * truncation methods should be replaced in the future by fixing the data model to use
 * a type-mapping for date properties that does not include time-of-day.)
 *
...
//                                                       propertyName);
  ...
  /**
   * Return true iff the caller of the method that is calling this method is from the
   * hibernate world. Specifically, we test the package name of the calling class for a
   * "org.hibernate." prefix.
   * @return true iff the caller of the method that is calling this method is from the
   * hibernate world
   */
  ...
  /**
   * Return the name of the calling class of the caller of the method of the caller of this
   * method that is calling this method. Specifically, we intend the caller of this method to
   * be {@link #isHibernateCaller()} or {@link #isTestCaller()}, and the caller of that method
   * to be an entity method that is trying to determine <i>its</i> caller.
   * @return the name of the calling class of the caller of the method of the caller of this
   * method that is calling this method
   */
