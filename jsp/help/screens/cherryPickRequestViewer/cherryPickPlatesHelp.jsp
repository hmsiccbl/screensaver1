...
<f:subview id="cherryPickPlatesHelpText">
	...
	<t:panelGroup visibleOnUserRole="cherryPickAdmin">
		<f:verbatim escape="false">
			...
			<p>Plate Mapping Files are fed directly to the plating machines
			in the lab, and specify how reagent from source wells (of cherry pick
			...
			just a subset of the plate mapping files. <i>Tip:</i> Downloading a
			subset of the Plate Mapping Files may be convenient if you intend to
			create subsets of Cherry Pick Plates at different times, or if you
			are reattempting the creation of a subset of plates that previously
			failed to be created. To download the files, select the desired rows
			in the Cherry Pick Plates data table, and then click "Download Files
			for Selected Plates". This will download a ZIP file, containing one
			mapping file per Cherry Pick Plate, plus an additional
			"instructions.txt" file. The instructions file lists the Cherry Pick
			Plates, along with their respective statuses. It also indicates
			whether particular Cherry Pick Plates require special treatment, such
			as when:
			<ul>
			...
			doing the following:
			<ol>
				<li>Select the Cherry Picks Plates in the data table by
				clicking on the respective checkbox in each row. <i>Tip:</i> The
			...
			<i>Tip:</i> For each plate you record as 'Failed', a new "attempt" will be created
for each Cherry Pick Plate, duplicating all of the Lab Cherry Picks on that
plate, maintaining the same layout, and reserving additional reagent.  However, it is
possible that some (or all) of the newly created Lab Cherry Picks will be
unfulfilled, due to lack of reagent.  You can invoke the "New Cherry Pick
Request for Unfulfilled" command, above, to handle these Lab Cherry Picks in
the future, as an entirely new Cherry Pick Request.
    </p>
		</f:verbatim>
	</t:panelGroup>
</f:subview>
