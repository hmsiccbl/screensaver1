...
testConfigureDbMultichannelSlog <- function(debug=FALSE) {
	## 1. PREPARE INPUT
	source("../inst/unitTests/makeDummies.R")
	testSet <- makeTestSet4(TRUE)
	...
	#3.1	plateConf: a 'data.frame' containing what was read from input file'confFile' (except the first two header rows).
	plateConf <- rc@plateConf
	plateConfTarget <- testSet$conf
	# For the * a space has been placed
	plateConfTarget[plateConfTarget=="*"] <- " *" 
	...
	#3.3  screenDesc: object of class 'character' containing what was read from input file 'descripFile'.
	#TODO
	
	#3.4 state: the processing status of the 'cellHTS' object is updated in to 'state["configured"]=TRUE'.
	checkEquals(rc@state[["configured"]],TRUE)
